...
export default async function SupportDashboardPage() {
  ...
  return (
    <div className="flex flex-col gap-8">
      <div>
        <h1 className="text-2xl font-bold text-[#001353] tracking-[-0.5px]">Support Dashboard</h1>
        <p className="text-sm text-[#5b6480] mt-1">Manage customer tickets and inquiries</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white rounded-[20px] border border-[#ddddee] p-5">
          <p className="text-sm text-[#5b6480]">My Open Tickets</p>
          <p className="text-2xl font-bold text-[#001353] mt-1">{myOpen}</p>
          <p className="text-xs text-amber-600 mt-1">Requires action</p>
        </div>
        <div className="bg-white rounded-[20px] border border-[#ddddee] p-5">
          <p className="text-sm text-[#5b6480]">Resolution Rate</p>
          <p className="text-2xl font-bold text-[#001353] mt-1">
            {totalTickets > 0 ? Math.round(((totalTickets - openTickets) / totalTickets) * 100) : 0}%
          </p>
          <div className="h-1.5 bg-[#ddddee] rounded-full mt-2"><div className="h-full bg-[#1a8f5c] rounded-full" style={{ width: `${totalTickets > 0 ? Math.round(((totalTickets - openTickets) / totalTickets) * 100) : 0}%` }} /></div>
        </div>
      </div>

      <div className="bg-white rounded-[24px] border border-[#ddddee] p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="font-bold text-[#001353]">Recent Tickets</h2>
          <Link href="/support/tickets" className="text-sm text-[#034795] font-medium">View all →</Link>
        </div>
            ...
            <tbody className="divide-y divide-[#ddddee]">
              {recentTickets.map((t) => (
                <tr key={t.id} className="hover:bg-[#f5f5fa]">
                  ...
                  <td className="py-3 text-sm text-[#5b6480]">{new Date(t.createdAt).toLocaleDateString()}</td>
                  <td className="py-3">
                    <Link href={`/support/tickets/${t.id}`} className="text-xs text-[#034795] font-bold hover:underline">
                      Reply →
                    </Link>
                  </td>
                </tr>
