  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Total Revenue" value={`৳${(Number(totalRevenue._sum.amount ?? 0) / 1000).toFixed(0)}K`} icon="💰" color="#fd6b22" change="+8% vs last month" changeType="up" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Monthly revenue */}
        <div className="bg-white rounded-[24px] border border-[#e8e6ea] p-6">
          <h2 className="font-bold text-[#1b1d21] mb-6">Monthly Revenue (৳)</h2>
          <div className="flex items-end gap-3 h-40">
            {months.map(({ label }, i) => {
              ...
              return (
                <div key={label} className="flex-1 flex flex-col items-center gap-2">
                  <span className="text-[10px] font-bold text-[#1b1d21]">৳{(amount / 1000).toFixed(0)}K</span>
                  <div className="w-full bg-[#f3f6f8] rounded-t-[8px] relative" style={{ height: "120px" }}>
              ...
              return (
                <div key={cat.name}>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="font-bold" style={{ color: cat.color }}>৳{cat.total.toLocaleString()}</span>
                  </div>
                  <div className="h-1.5 bg-[#f3f6f8] rounded-full overflow-hidden">
